...
module Cafeznik
  class ThorCLI < Thor
    ...
    def default
      ...
      tree = repo ? github_tree : local_tree
      selected = select_files(tree)
      copy_files_to_clipboard(selected, options[:no_header], options[:with_tree], tree)
    end
    ...
    def github_token
      @_github_token ||= ENV['GITHUB_TOKEN'] || fetch_gh_token || (log.error("GitHub token not found") && exit(1))
    end
    ...
    def client
      @_client ||= Octokit::Client.new(access_token: github_token, auto_paginate: true)
    end
    ...
    def files_in_directory(dir_path)
      repo_tree.select { _1.start_with?("#{dir_path}/") }.reject(&method(:directory?))
    end

    def directory?(path) = path.end_with?('/')

    def copy_files_to_clipboard(selected_files, no_header, with_tree, tree)
